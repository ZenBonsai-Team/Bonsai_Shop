 ...
 * + Xác nhận thanh toán phần còn lại (Confirm remaining payment), hoàn tất đơn
 * (Complete), hủy do khách không nhận (Customer No-Show), từ chối đơn (Reject).
 *
 ...
 * - [Moderator xem chi tiết đơn] → GET /api/orders/{orderCode} →
 * getOrderDetail()
 * - [Moderator nhận đơn] → POST /api/orders/{orderCode}/claim → claimOrder()
 * - [Moderator trả đơn về Pool] → POST /api/orders/{orderCode}/unclaim →
 * unclaimOrder()
 * - [Moderator duyệt đơn & áp phí] → POST /api/orders/{orderCode}/verify →
 * verifyOrder()
 * - [Moderator xác nhận thu đủ tiền đợt 2] → POST
 * /api/orders/{orderCode}/confirm-remaining-payment → confirmRemainingPayment()
 * - [Moderator hoàn thành đơn đã thanh toán 100%] → POST
 * /api/orders/{orderCode}/complete → completePaidOrder()
 * - [Moderator hủy đơn vì khách không nhận] → POST
 * /api/orders/{orderCode}/customer-no-show → markCustomerNoShow()
 * - [Moderator từ chối đơn] → POST /api/orders/{orderCode}/reject →
 * rejectOrder()
 *
 ...
 * - PurchaseOrderRequestDTO, OrderResponseDTO, PasswordResetOtp, Order,
 * Product, Payment
 */
@RestController
...
public class OrderApiController {
     ...
     * - Người dùng thực hiện: Nhập Email tại bước thanh toán (Checkout) khi chưa
     * đăng nhập tài khoản và bấm "Gửi mã xác nhận" / "Lấy mã OTP".
     ...
     * + productIds (List<Integer>): Danh sách ID cây khách đang chọn mua để kiểm
     * tra trước tính khả dụng.
     *
     * Điều phối xử lý:
     * 1. Controller kiểm tra định dạng email và parse danh sách productIds.
     * 2. Nếu có productIds, gọi OrderService.getProductsByIds() và
     * OrderService.validateProductAvailability() để kiểm tra nhanh trạng thái
     * (Fail-fast nếu cây không AVAILABLE).
     * 3. Kiểm tra rate-limit chống spam qua
     * RegisterOtpRepository.findTopByEmailOrderByCreatedAtDesc() (cooldown 60
     * giây).
     * 4. Sinh mã OTP 6 chữ số ngẫu nhiên.
     * 5. Gửi email chứa OTP trước qua EmailService.sendGuestOrderOtpOrThrow().
     * 6. Sau khi gửi email thành công, xóa OTP cũ qua
     * RegisterOtpRepository.deleteByEmail() và lưu bản ghi PasswordResetOtp mới với
     * thời hạn 5 phút.
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK (Thành công), 400 Bad Request (Email sai/Cây không
     * còn), 429 Too Many Requests (Spam trong 60s), 500 Internal Server Error (Lỗi
     * gửi mail).
     * - Response body: Map<String, Object> ("success", "message",
     * "retryAfterSeconds", "unavailableProducts").
     * - Ý nghĩa dữ liệu frontend nhận được: Thông báo để hiển thị đếm ngược
     * cooldown hoặc báo khách nhập mã OTP từ hộp thư.
     *
     * Tác động dữ liệu:
     * - Bảng/Entity bị đọc: PRODUCT (kiểm tra status), PASSWORD_RESET_OTP (kiểm tra
     * rate limit).
     * - Bảng/Entity bị ghi/cập nhật: PASSWORD_RESET_OTP (xóa cũ, lưu OTP mới hết
     * hạn sau 5 phút).
    ...
    public ResponseEntity<Map<String, Object>> sendGuestOtp(@RequestBody Map<String, Object> payload) {
        ...
        // [3] Pre-validate: load products từ DB + kiểm tra limit + kiểm tra
        // availability
        ...
        if (!productIds.isEmpty()) {
            ...
            // LƯU Ý: đây KHÔNG phải data guard. reserveIfAvailable() trong createOrder()
            // mới là lớp bảo vệ cuối cùng.
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Truy cập trang Kho đơn hàng chung (Orders Pool) để
     * tìm các đơn PENDING chưa có ai nhận để claim.
     ...
     * 2. Service gọi OrderRepository.searchOrdersPool(search, pageable) để lấy các
     * đơn có assignedTo IS NULL và orderStatus = 'PENDING'.
     * 3. Controller map danh sách Order entity sang OrderResponseDTO qua
     * convertToDTO().
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK
     * - Response body: Map chứa "orders" (List<OrderResponseDTO>), "totalCount",
     * "pages", "currentPage".
     *
     * Tác động dữ liệu:
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Xem danh sách tổng hợp đơn hàng theo bộ lọc trạng
     * thái.
     ...
     *
     * Dữ liệu nhận vào:
     * - Request params: search, status (ALL, PENDING, PENDING_PAYMENT, DEPOSITED,
     * PAID, COMPLETED, CANCELLED), sort, page, limit.
     *
     * Điều phối xử lý:
     * 1. Controller gọi OrderService.getFilteredOrders(search, status, sort, page,
     * limit)
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Truy cập trang "Đơn hàng của tôi" để theo dõi các đơn
     * mình đã nhận xử lý.
     ...
     * 2. Controller gọi OrderService.getMyOrders(moderatorId, search, status, sort,
     * page, limit).
     * 3. Service gọi OrderRepository.searchMyOrders(...) với assignedTo.userId =
     * moderatorId.
     *
     * Dữ liệu trả về:
     ...
     * 3. Đếm số lượng đơn theo các trạng thái PENDING, PENDING_PAYMENT, PAID,
     * CANCELLED thuộc về moderator.
     *
     * Dữ liệu trả về:
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Moderator bấm nút "Tiếp nhận đơn" (Claim) trên card
     * đơn hàng ở trang Orders Pool.
     ...
     * 4. Cập nhật Order: setAssignedTo(moderator), setAssignedAt(now). Lưu bằng
     * orderRepository.save().
     * 5. Tạo và lưu bản ghi OrderHandling mới (isActive = true) qua
     * orderHandlingRepository.save().
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK (Thành công), 401 (Chưa đăng nhập), 409 Conflict (Đơn
     * đã có người nhận hoặc sai trạng thái), 500 (Lỗi hệ thống).
     * - Response body: Map {"success": true, "message": "Nhận đơn hàng thành
     * công."}
     *
     * Tác động dữ liệu:
     ...
     * - Thay đổi trạng thái: Order vẫn giữ "PENDING" nhưng đã được gán người phụ
     * trách.
     */
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Moderator bấm "Trả về kho chung" trên trang My Orders
     * hoặc Order Detail khi không thể tiếp tục xử lý đơn chưa duyệt.
     ...
     * 2. Service kiểm tra quyền sở hữu (order.getAssignedTo() == moderator) và
     * trạng thái đơn phải là "PENDING".
     * 3. Set assignedTo = null, assignedAt = null trên Order.
     * 4. Cập nhật các bản ghi OrderHandling đang active: set isActive = false,
     * releasedAt = now.
     *
     * Dữ liệu trả về:
     ...
     * 2. Truy vấn đếm số lượng đơn toàn hệ thống theo PENDING, PENDING_PAYMENT,
     * PAID, CANCELLED.
     *
     * Dữ liệu trả về:
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Bấm vào xem chi tiết đơn hàng hoặc mở trang Order
     * Detail.
     ...
     * 2. Convert Order entity sang OrderResponseDTO (tính toán treePrice,
     * immediatePaymentAmount, remainingPaymentAmount, payments history, handling
     * history).
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK (hoặc 404 Not Found).
     * - Response body: OrderResponseDTO đầy đủ thông tin khách hàng, sản phẩm,
     * thanh toán.
     */
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Moderator liên hệ thỏa thuận với khách, sau đó nhập
     * Phí cẩu (Crane Fee), Phí vận chuyển (Shipping Fee), và Số tiền đặt cọc
     * (Deposit Amount) rồi bấm "Xác nhận duyệt đơn".
     ...
     * - Request body: Map<String, Object> payload (craneFee, shippingFee,
     * depositAmount)
     * - Authentication: Principal của Moderator
     *
     * Điều phối xử lý:
     * 1. Controller trích xuất và parse craneFee, shippingFee, depositAmount.
     * 2. Gọi OrderService.verifyOrder(orderCode, craneFee, shippingFee,
     * depositAmount, moderator).
     ...
     * - Nếu flow Đặt cọc (DEPOSIT/COD): kiểm tra depositAmount hợp lệ (<=
     * treePrice), tạo/cập nhật Payment record #1 với paymentType = "DEPOSIT",
     * amount = depositAmount, status = "PENDING".
     * - Nếu flow Thanh toán đủ (FULL_PAYMENT/VNPAY): set depositAmount = 0, tạo/cập
     * nhật Payment record #1 với paymentType = "FULL_PAYMENT", amount =
     * totalAmount, status = "PENDING".
     * 5. Cập nhật Order: orderStatus = "PENDING_PAYMENT".
     * 6. Ghi bản ghi OrderLog (actionType = "VERIFY", fromStatus = "PENDING",
     * toStatus = "PENDING_PAYMENT").
     * 7. Đóng bản ghi OrderHandling hiện tại (isActive = false, releasedAt = now).
     * 8. Bắn sự kiện OrderVerifiedEvent → OrderEventListener bắt sự kiện và gọi
     * MailService.sendOrderApprovedEmail() gửi email kèm link thanh toán VNPay cho
     * khách.
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK, 400 Bad Request, 403 Forbidden.
     * - Response body: Map {"success": true, "message": "Duyệt đơn hàng thành
     * công."}
     *
     * Tác động dữ liệu:
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Đơn hàng đã ở trạng thái DEPOSITED (khách đã cọc
     * thành công). Khi giao cây đến nhà và thu tiền mặt/chuyển khoản phần còn lại,
     * Moderator bấm "Xác nhận thu đủ tiền".
     ...
     *
     * Điều phối xử lý:
     * 1. Controller gọi OrderService.confirmRemainingPayment(orderCode, notes,
     * moderator).
     * 2. Service kiểm tra trạng thái đơn phải là "DEPOSITED" và đúng Moderator phụ
     * trách.
     * 3. Tính tiền còn lại: remainingAmount = totalAmount - depositPaid.
     * 4. Tạo Payment record #2: paymentType = "REMAINING_PAYMENT", paymentMethod =
     * "CASH", paymentStatus = "SUCCESS", amount = remainingAmount, paymentDate =
     * now.
     * 5. Cập nhật Order: orderStatus = "COMPLETED", completedAt = now.
     * 6. Cập nhật trạng thái tất cả cây trong đơn sang "SOLD" qua
     * markProductsAsSold().
     * 7. Ghi nhận sổ cái doanh thu hoàn tất qua
     * FinancialLedgerService.recordCompletedOrderRevenueIfAbsent().
     * 8. Ghi OrderLog (actionType = "REMAINING_PAYMENT_CONFIRMED", fromStatus =
     * "DEPOSITED", toStatus = "COMPLETED").
     * 9. Bắn sự kiện OrderPaidEvent để gửi hóa đơn hoàn tất qua email.
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK, 400, 409.
     * - Response body: Map {"success": true, "message": "Xác nhận đã thanh toán đầy
     * đủ thành công!"}
     *
     * Tác động dữ liệu:
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Đơn hàng ở trạng thái "PAID" (khách đã thanh toán
     * 100% qua VNPay), sau khi giao cây thành công, Moderator bấm "Hoàn tất đơn
     * hàng".
     * - Màn hình/chức năng: Moderator - Order Detail.
     *
     * API:
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Đơn hàng ở trạng thái DEPOSITED, khi giao cây khách
     * từ chối nhận vô lý hoặc không liên lạc được, Moderator bấm "Khách không nhận
     * hàng".
     * - Màn hình/chức năng: Moderator - Order Detail.
     *
     * API:
     ...
     *
     * Điều phối xử lý:
     * 1. Controller gọi OrderService.markDepositedOrderCustomerNoShow(orderCode,
     * notes, moderator).
     * 2. Service kiểm tra trạng thái đơn phải là "DEPOSITED".
     * 3. Ghi nhận giữ lại tiền cọc (Forfeited Deposit Income) vào sổ cái tài chính
     * FinancialLedger.
     ...
     * Dữ liệu trả về:
     * - HTTP status: 200 OK
     * - Response body: Map {"success": true, "message": "Đã hủy đơn vì khách không
     * nhận. Tiền cọc không hoàn."}
     *
     * Tác động dữ liệu:
    ...
    public ResponseEntity<Map<String, Object>> markCustomerNoShow(
        ...
        try {
            ...
            response.put("message",
                    success ? "Đã hủy đơn vì khách không nhận. Tiền cọc không hoàn." : "Thao tác thất bại.");
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            response.put("success", false);
            response.put("message", e.getMessage());
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Moderator kiểm tra đơn hàng PENDING nhưng không hợp
     * lệ (ví dụ: địa chỉ không hỗ trợ, cây bị lỗi, khách đổi ý trước khi duyệt) và
     * bấm "Từ chối duyệt đơn" kèm lý do.
     ...
     * 4. Giải phóng toàn bộ cây trong đơn từ "RESERVED" về "AVAILABLE" trong
     * PRODUCT.
     * 5. Ghi OrderLog (actionType = "REJECT", fromStatus = "PENDING", toStatus =
     * "CANCELLED").
     * 6. Đóng bản ghi OrderHandling (isActive = false, releasedAt = now).
     * 7. Bắn sự kiện OrderRejectedEvent → OrderEventListener gọi
     * MailService.sendOrderRejectedEmail() gửi email thông báo hủy đơn kèm lý do
     * cho khách.
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK, 400 Bad Request, 403 Forbidden.
     * - Response body: Map {"success": true, "message": "Từ chối duyệt đơn hàng
     * thành công."}
     *
     * Tác động dữ liệu:
     * - ORDER: orderStatus: PENDING → CANCELLED
     * - PRODUCT: productStatus: RESERVED → AVAILABLE (Cây mở bán lại trên
     * Marketplace)
     ...
     *
     * Khi nào được gọi trên web:
     * - Người dùng thực hiện: Điền đầy đủ thông tin nhận hàng, chọn phương thức
     * thanh toán (DEPOSIT / FULL), nhập mã OTP (nếu là Guest) và bấm "Đặt hàng
     * ngay".
     ...
     * + paymentMethod (String): "DEPOSIT" (Đặt cọc) hoặc "FULL" / "VNPAY" (Thanh
     * toán 100%)
     ...
     *
     * Điều phối xử lý:
     * 1. Chặn tài khoản nhân viên/admin (OWNER, ARTISAN, MODERATOR, ADMIN) không
     * cho phép tự đặt hàng.
     * 2. Pre-validate trạng thái cây (nếu đã đăng nhập): kiểm tra cây còn AVAILABLE
     * không.
     * 3. Xác thực OTP nếu là Guest:
     * - Tìm OTP mới nhất trong RegisterOtpRepository theo email.
     * - Kiểm tra OTP chưa dùng (isUsed = false), còn hạn (expiredAt > now), đúng mã
     * (otpCode).
     * - Đánh dấu OTP đã dùng (isUsed = true) và lưu lại.
     * 4. Gọi OrderService.createOrder(dto, customer):
     * - Load sản phẩm từ DB.
     * - Thực hiện giữ chỗ nguyên tử (Atomic Reserve) từng cây qua
     * productRepository.reserveIfAvailable(productId).
     * - Cập nhật productStatus của cây sang "RESERVED".
     * - Tạo Order entity với orderStatus = "PENDING", orderType = "ONLINE", mã đơn
     * sinh dạng "BSMS-XXXXXX".
     * - Tạo danh sách OrderDetail lưu snapshot giá mua (priceAtPurchase =
     * product.price).
     * - Lưu Order vào ORDER table.
     * - Khởi tạo 1 bản ghi Payment ban đầu với paymentStatus = "PENDING"
     * (paymentType = DEPOSIT hoặc FULL_PAYMENT theo lựa chọn của khách).
     ...
     *
     * Dữ liệu trả về:
     * - HTTP status: 200 OK, 400 Bad Request, 403 Forbidden, 500 Internal Server
     * Error.
     * - Response body: Map {"success": true, "paymentMethod": "DEPOSIT",
     * "orderCode": "BSMS-ABC123"}
     *
     * Tác động dữ liệu:
     ...
     * + PRODUCT: productStatus: AVAILABLE → RESERVED (giữ chỗ không cho người khác
     * mua)
     ...
     * - Thay đổi trạng thái: Order: null → PENDING; Product: AVAILABLE → RESERVED;
    ...
    private OrderResponseDTO convertToDTO(Order order) {
        ...
        if (order.getPayments() != null && !order.getPayments().isEmpty()) {
            ...
            isDepositFlow = order.getPayments().stream().anyMatch(p -> "DEPOSIT".equalsIgnoreCase(p.getPaymentType()) ||
