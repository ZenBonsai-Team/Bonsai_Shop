 ...
 * Kết nối Controller -> Service -> Repository -> Database để kiểm tra luồng chốt đơn trên livestream.
 */
@SpringBootTest
...
public class LiveStreamIntegrationTest {
    ...
    void setUp() {
        ...
        Variety var = varietyRepository.save(Variety.builder().varietyName("Tùng La Hán").category(cat).build());
        ProductSegment seg = productSegmentRepository.save(ProductSegment.builder().segmentName("Cao Cấp").build());
    ...
    void testProcessChat_ChotDon_CreatesLiveLeadInDatabase() throws Exception {
        ...
        // Kiểm chứng DB State: Hệ thống tự động phân tích cú pháp tạo ra khách hàng tiềm năng chốt đơn (LiveLead) trong DB
